...
function getOtherProp(deal, fieldName) {
  for (const p of (deal.OtherProperties||[])) {
    if ((p.FieldName||p.FieldKey||'').trim().toLowerCase()===fieldName.toLowerCase())
...
function extractProduct(deal) {
  const v = getOtherProp(deal, CAMPO_PRODUTO); if (v) return v;
  const t = (deal.Title||'').toUpperCase();
  if (t.includes('RSS')||t.includes('RESIDUO')) return 'RSS';
...
function extractOrigin(deal) {
  ...
  return getOtherProp(deal, CAMPO_ORIGEM) || 'Não informado';
}

function extractLossReason(deal) {
  return deal.LossReasonName || deal.LossReason || 'Não informado';
}
...
const openDeals = await ploomesFetch(`/Deals?$filter=StatusId eq 1`, key);
    ...
    for (const deal of wonDeals) {
      ...
      const month = getMonth(deal.WonDate), year = getYear(deal.WonDate);
      if (!month || year !== ANO) continue;
      const val = deal.TotalValue||0, origin = extractOrigin(deal), product = extractProduct(deal);
    ...
    const sellers = Object.keys(SELLER_CONFIG).map(name => {
      ...
      const topDeals=ob.deals.sort((a,b)=>(b.TotalValue||0)-(a.TotalValue||0)).slice(0,7).map(d=>[
        (d.Title||'Sem título').substring(0,40).toUpperCase(),d.DealStageName||'Pipeline',
        d.TotalValue||0,daysSince(d.CreateDate),extractProduct(d),alertLevel(d)
      ]);
      return {
    ...
    [1,2,3].forEach(m=>{
      ...
      mbs[m]=sellers.map(s=>{
        ...
        const r=sdata[rK]||0,w=sdata[wK]||0,l=sdata[lK]||0;
    ...
    const allWon=sellers.reduce((acc,s)=>acc+s.total_won,0),allLost=sellers.reduce((acc,s)=>acc+s.total_lost,0);
    const mrev={1:sellers.reduce((acc,s)=>acc+s.jan,0),2:sellers.reduce((acc,s)=>acc+s.feb,0),3:sellers.reduce((acc,s)=>acc+s.mar,0)};
