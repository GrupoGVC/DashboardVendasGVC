// api/ploomes.js — Vercel Serverless Function (otimizado)
...
const DATA_INI      = `${ANO}-01-01T00:00:00`;
const DATA_FIM      = `${ANO}-12-31T23:59:59`;
...
async function ploomesGet(url, key) {
  ...
  throw new Error('Ploomes: muitas tentativas (429)');
}

// ── Paginação sequencial ──────────────────────────────────────────────────────
async function ploomesFetch(filter, key) {
  let all = [], skip = 0;
  const top = 100;
  while (true) {
    const url = `${BASE}/Deals?$filter=${encodeURIComponent(filter)}&$top=${top}&$skip=${skip}&$select=Id,Title,TotalValue,StatusId,DealStageName,ResponsibleUserName,WonDate,LostDate,CreateDate,LossReasonName,OriginName`;
    const json  = await ploomesGet(url, key);
...
function extractOrigin(deal) {
  return deal.OriginName || 'Não informado';
}

function extractLossReason(deal) {
  return deal.LossReasonName || 'Não informado';
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Busca sequencial com filtros de data para minimizar volume
    const wonDeals  = await ploomesFetch(`StatusId eq 2 and WonDate ge datetime'${DATA_INI}' and WonDate le datetime'${DATA_FIM}'`, key);
    const lostDeals = await ploomesFetch(`StatusId eq 3 and LostDate ge datetime'${DATA_INI}' and LostDate le datetime'${DATA_FIM}'`, key);
    const openDeals = await ploomesFetch(`StatusId eq 1`, key);
    ...
    // ── Processar ABERTOS ─────────────────────────────────────────────────────
    const funnelCount={}, funnelVal={}, openBySel={};
    let fcTotal=0;
    for (const deal of openDeals) {
      ...
      if (name) {
        ...
        if (['forcast','forecast'].includes(stage.toLowerCase())) {
          openBySel[name].forecast+=val; fcTotal+=val;
        }
      }
    }
    ...
    const sellers = Object.keys(SELLER_CONFIG).map(name => {
      ...
      const topDeals = ob.deals
        .sort((a,b)=>(b.TotalValue||0)-(a.TotalValue||0)).slice(0,7)
      ...
      return {
        id:conf.id, name, empresa:conf.empresa,
